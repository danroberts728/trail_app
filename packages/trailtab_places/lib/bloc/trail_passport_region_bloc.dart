...
/// A BLoC for Passport Region
class TrailPassportRegionBloc {
  /// A reference to the central database
  TrailDatabase _db;

  /// The current region for the widget
  TrailRegion region;

  /// The current trail places that exist in this region
  List<TrailPlace> _places;

  /// The user's current check ins for places in this region
  ...
  /// Default constructor
  TrailPassportRegionBloc(TrailDatabase db, {this.region})
      : assert(db != null) {
    _db = db;
    _places = _db.places.where((p) => region.places.contains(p.id)).toList();
    _checkIns =
        _db.checkIns.where((c) => region.places.contains(c.placeId)).toList();
  ...
  void _buildAndSendStampedInformation() {
    ...
    places.forEach((p) {
      var thisPlaceCheckIns =
          _checkIns.where((c) => c.placeId == p.id).toList();
  ...
  void _onPlacesUpdate(List<TrailPlace> event) {
    _places = event.where((p) => region.places.contains(p.id)).toList();
    _buildAndSendStampedInformation();
  }

  /// Callback when check ins are updated
  void _onCheckInUpdate(List<CheckIn> event) {
    _checkIns = event.where((c) => region.places.contains(c.placeId)).toList();
    _buildAndSendStampedInformation();
  }
